...
1. The binary data of the output `.pdf` file can change quite drastically, even after making small changes to `.tex` source. This makes it quite useless to use something like `git diff` directly on the PDF. Instead, a tool like #link("https://ctan.org/pkg/latexdiff")[latexdiff] should be used.
